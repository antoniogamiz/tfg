...
Este trabajo gira en torno al algoritmo \textit{Word2Vec} \textbf{ref}, una técnica de procesamiento
...
que mejoran a partir de la experiencia. Tom M. Mitchell \textbf{ref} enunció una definición formal
del término:
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \draw[-stealth] (input1) -- node[above, xshift=1cm, yshift=-0.5cm] {$w_3$} (hidden2);
...
Los pesos no son más que números arbitrarios, que son actualizados en cada fase de aprendizaje (\textbf{ref backprop section}).
...
\begin{figure}[H]
    \includegraphics[width=5cm]{sigmoind.png}
    \centering
\end{figure}

El principal problema de esta función se denomina \textit{vanishing problem} \textbf{ref}. Cuando se avance un poco más en los fundamentos de las redes neuronales, se volverá a este problema.
...
de forma que el error definiedo en \ref{eqn:3}, sea mínimo. Por lo tanto, la expresión puede ser reescrita:
...
donde una vez más $y$ es el vector de salida, $t$ el vector que se quiere y $x$ el vector de entrada.
Esta expresión se ha obtenido usando la regla de la cadena derivando $E=\frac{1}{2}(t-y)^2$:
...
donde se han usado las expresiones \ref{sigmoid:1}, \ref{sigmoid:2} y \ref{eqn:2}.
